...
| Release governance: one platform version across every deployment unit with the committed API contract named as the compatibility surface (ADR-021), a published licence (Apache-2.0), a coordinated vulnerability-disclosure policy, a human-readable release record, and promotion as an authorized act on a verified, digest-pinned artifact | Delivered (mechanism exercised, not operated — no environment is hosted; approval is the hosting platform's environment protection, and rollback stays deployment-level while migrations are forward-only) |
| Release 1.0: readiness evaluated against the repository, and the release path rehearsed end to end from **published, digest-pinned images** on the staging overlay — TLS edge, JWT identity, retention enforcement, crypto-shredding payloads, durable audit | Delivered (the rehearsal found and fixed a containerized evidence-upload defect; the run reached a terminal `exhausted` state under a slow provider rather than synthesizing an outcome, which is documented loop behaviour) |
...
| 1.11.0 | 2026-07-26 | Delivery Record gains **release governance** (ES-072): release identity and compatibility (ADR-021), Apache-2.0 licence, disclosure policy, changelog, and verified digest-pinned promotion — delivered as a mechanism, with the unhosted-environment limitation stated rather than implied |
| 1.12.0 | 2026-07-26 | Delivery Record gains the **Release 1.0 readiness and rehearsal** row (ES-073): the release path exercised from published digest-pinned artifacts through the production-shaped edge, with the defect it uncovered fixed and the loop's terminal state reported as observed rather than as hoped |
